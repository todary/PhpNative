...
class SortMethodTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function setUp()
    {
        ...
        $data = $hotels['hotels']??[];
        $this->nameServiceObject = new NameService($data, 'Media One Hotel');
        $this->nameServiceObject->search();
        $this->nameSortObject = new NameSort($this->nameServiceObject);
        $this->priceSortObject = new PriceSort($this->nameServiceObject);
    }

    public function testNameSort()
    {
        $this->assertEquals($this->nameSortObject->sort(), $this->nameServiceObject->getAfterSort());
    }

    public function testPriceSort()
    {
        $this->assertEquals($this->priceSortObject->sort(), $this->nameServiceObject->getAfterSort());
    }
}
