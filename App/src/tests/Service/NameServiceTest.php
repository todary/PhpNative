...
//        $hotels = file_get_contents('https://api.myjson.com/bins/tl0bp');
        ...
        $this->nameServiceObject = new SearchNameService($this->data,'Rotana Hotel');
    }

    public function testSearchNameService()
    {
        $this->assertEquals($this->nameServiceObject->search(), $this->nameServiceObject->getAfterSearch());
    }


    public function testSetNameService()
    {
        $this->nameServiceObject->setAfterSearch([]);
        $this->assertEquals([], $this->nameServiceObject->getAfterSearch());
    }

    public function testSetSortNameService()
    {
        $this->nameServiceObject->setAfterSort([]);
        $this->assertEquals([], $this->nameServiceObject->getAfterSort());
    }
}
