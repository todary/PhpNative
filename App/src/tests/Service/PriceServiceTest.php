...
//        $hotels = file_get_contents('https://api.myjson.com/bins/tl0bp');
        ...
        $this->priceServiceObject = new SearchPriceService($this->data,10,100);
    }

    public function testSearchNameService()
    {
        $this->assertEquals($this->priceServiceObject->search(), $this->priceServiceObject->getAfterSearch());
    }


    public function testSetNameService()
    {
        $this->priceServiceObject->setAfterSearch([]);
        $this->assertEquals([], $this->priceServiceObject->getAfterSearch());
    }

    public function testSetSortNameService()
    {
        $this->priceServiceObject->setAfterSort([]);
        $this->assertEquals([], $this->priceServiceObject->getAfterSort());
    }
}
