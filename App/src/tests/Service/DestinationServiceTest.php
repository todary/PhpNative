...
//        $hotels = file_get_contents('https://api.myjson.com/bins/tl0bp');
    ...
    {
        $this->assertEquals($this->destinationServiceObject->search(), $this->destinationServiceObject->getAfterSearch());
    }


    public function testSetNameService()
    {
        $this->destinationServiceObject->setAfterSearch([]);
        $this->assertEquals([], $this->destinationServiceObject->getAfterSearch());
    }

    public function testSetSortNameService()
    {
        $this->destinationServiceObject->setAfterSort([]);
        $this->assertEquals([], $this->destinationServiceObject->getAfterSort());
    }
}
